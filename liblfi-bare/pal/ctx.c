...
static struct Sys *sysalloc(struct LFIPlatform *plat, uintptr_t base) {
  struct Sys *sys;
...
syssetup(struct LFIPlatform* plat, struct Sys* sys, uintptr_t base)
{
    ...
    int err = mprotect((void*) base, plat->opts.pagesize, PROT_READ);
    assert(err == 0);
}
...
EXPORT void
lfi_ctx_tpset(struct LFIContext* ctx, uintptr_t tp)
{
    ctx->tp = tp;
}
